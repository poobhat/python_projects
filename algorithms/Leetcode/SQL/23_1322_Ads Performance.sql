...
else, ctr = ad total clicks / (ad total clicks + ad total views)
...
-----------------------------------------------------------------------------------------------
-- Runtime: 669 ms, faster than 20.40%
select
	ad_id,
	round(case when sum(action = 'Clicked')+ sum(action = 'Viewed')= 0 then 0
	else sum(action = 'Clicked')* 100 /(sum(action = 'Clicked')+ sum(action = 'Viewed')) end,2) as ctr
from
	ads
group by
	ad_id
...
--Runtime: 498 ms, faster than 66.75%
