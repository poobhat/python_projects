...
Return the sum of the numbers from index left to index right (indexed from 1), inclusive, in the new
array. Since the answer can be a huge number return it modulo 109 + 7.
...
class Solution:
    def rangeSum(self, nums: [int], n: int, left: int, right: int) -> int:
        ...
        while start < len(nums):
            ...
            start+=1
        sumList.sort()
        return (sum(sumList[left-1:right])) % (10**9+7)
