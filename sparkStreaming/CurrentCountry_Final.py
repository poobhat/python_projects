...
spark = SparkSession.builder.appName("CurrentCountry").getOrCreate()
...
customerDF = zSetEntriesDecodedDF1.withColumn("customer", from_json("redisEvent",customerSchema)) \
    .select(col("customer.*")) \
...
customerLocationDF = zSetEntriesDecodedDF1.withColumn("customerLocation", from_json("redisEvent",customerSchema)) \
...
customerSelectDF = spark.sql("select accountNumber as customerAccountNumber, location as homeLocation, birthDay from Customer where birthDay is not null")\
    .select("customerAccountNumber", "homeLocation", split("birthDay", "-").getItem(0).alias("birthYear"))
...
customerLocationSelectDF = spark.sql("select accountNumber as locationAccountNumber,  location from CustomerLocation")

#TO-DO: join the customer and customer location data using the expression: customerAccountNumber = locationAccountNumber
currentCountryDF = customerSelectDF.join(customerLocationSelectDF, expr("""
customerAccountNumber = locationAccountNumber
""")).filter("location != homeLocation")

# TO-DO: write the stream to the console, and configure it to run indefinitely
currentCountryDF.writeStream.outputMode("append").format("console").start().awaitTermination()
