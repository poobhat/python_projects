...
spark=SparkSession.builder.appName("BankDeposits").getOrCreate()
...
bankDepositsJSONDF = bankDepositsDF.withColumn("value", from_json("value", bankDepositsSchame)) \
    .select(col("value*"))
# TO-DO: create a temporary streaming view called "BankDeposits"
# it can later be queried with spark.sql
bankDepositsJSONDF.createOrReplaceTempView("BankDeposits")

#TO-DO: using spark.sql, select * from BankDeposits
bankDepositsFinalDF = spark.sql("select * from BankDeposits")
...
bankDepositsFinalDF.writeStream.format("console")\
    .option("kafka.bootstrap.servers", "localhost:9092")\
    .option("startingOffsets", "earliest")\
    .start()\
    .awaitTermination()
